...
Function slope_corr, height, slope
	...
	if n_elements(height) gt 1 then begin
		...
		if (count1 gt 0) then begin
			coeff[index1] = float(slope[index1]-scale1)/float(scale2-scale1)*float(slope[index1])/100000.
		endif
		if (count2 gt 0) then begin
			coeff[index2] = float(slope[index2])/100000.
		endif

		return, height * (1.-coeff)

	endif else begin
		coeff = 0.
		if (slope ge scale1) then begin
			if (slope le scale2) then begin
				coeff = float(slope-scale1)/float(scale2-scale1) * float(slope)/100000.
				return, height * (1.-coeff)
			endif
			coeff = float(slope)/100000.
		endif
		return, height * (1.-coeff)
	endelse
